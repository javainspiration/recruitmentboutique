...
try {
  ...
  //set order detail
  $orderItems = json_decode($_POST['order']);
  $items = array();
  $moneys = array();
  foreach ($orderItems as $key => $item) {
    $moneys[$key] = new Money();
    $moneys[$key]->setCurrency('GBP');
    $moneys[$key]->setAmount($item->price*100);

    $items[$key] = new OrderLineItem($item->qty);
    $items[$key]->setName($item->name);
    $items[$key]->setBasePriceMoney($moneys[$key]);
  }

  // $money_A = new Money();
  // $money_A->setCurrency('GBP');
  // $money_A->setAmount(0);
  ...
  $checkout_request->setRedirectUrl($_POST['redirect_url']);
...
echo $response->getResult()->getCheckout()->getCheckoutPageUrl();
